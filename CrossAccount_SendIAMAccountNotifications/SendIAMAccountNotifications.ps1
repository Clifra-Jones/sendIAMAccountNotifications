...
#Requires -Modules @{ModuleName='Aws.Tools.SecurityToken'; ModuleVersion='4.1.105'}
...
#Assume the role Manage_Secrets_Role and get credentials for that role.
Write-Host "Assuming Role, Manage_Secrets_Role"

$msrResponse  = (Use-STSRole -Region us-east-1 -RoleArn "arn:aws:iam::268928949034:role/Manage_Secrets_Role" -RoleSessionName RSN).Credentials
$msrCredentials = New-AWSCredentials -AccessKey $msrResponse.AccessKeyId -SecretKey $msrResponse.SecretAccessKey -SessionToken $msrResponse.SessionToken

# Retrieve the SES credentials from Secrets Manager.
Write-Host "Retrieving SES_SMTP_User access Keys from Secrets Manager."
$SES_Creds = (Get-SECSecretValue -SecretId 'SES_SMTP_User' -Credential $msrCredentials).SecretString | ConvertFrom-Json
...
function RotateKeys() {
    ...
    switch ($Action) {
        'New' {
            ...
            try {
                Update-SECSecret -SecretId $SecretNameURN -SecretString $AccessKeysJSON -Credential $msrCredentials
            } catch {
                throw $_
            }
...
foreach ($iamUser in $iamUsers) {
    ...
    if ($iamUserInfo[5] -eq 'root') {
        ...
        If ($tags.Count -gt 0) {
            If ($tags.Key.IndexOf("SecretName") -gt -1) {
                ...
                Write-Host "Secret name no define on IAM user. Exiting"
                Continue
            }
        }
    ...
    if ($iamUserEmail -notin $ExcludeUsers) {
        ...
        #Check if we are forcefully regenerating new keys for this user
        if ($tags.Count -gt 0) {
        ...
        foreach ($AccessKey in $AccessKeys) {
            ...
            [datetime]$deleteDate = $AccessKey.CreateDate.AddDays(111)
            ...
            if ($accessKey.Status -eq "Inactive" -and ($CurrentDate -ge $deleteDate)) {
                ...
                $secret = Get-SECSecret -SecretId $secretName -AccessKey $msrCredentials -Credential $msrCredentials
        ...
        if ($sendEmail) {
            ...
            #comment for testing
            Send-MailMessage -SmtpServer $SMTPServer -To $iamUserEmail -From $From -Subject $Subject -Body $body -UseSsl -Credential $smtpCreds -Port 587 -WarningAction SilentlyContinue
